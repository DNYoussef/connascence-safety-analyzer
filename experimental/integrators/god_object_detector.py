#!/usr/bin/env python3
"""
God Object Detector - Enhanced Architectural Quality Assessment Tool
Identifies classes and modules that violate Single Responsibility Principle
using cohesion-based scoring and statistical outlier detection.
"""
...
# Import enhanced cohesion analyzer
try:
    from ..analyzer.cohesion_analyzer import (
        StatisticalGodObjectDetector, GodObjectFinding, ClassRole
    )
    ENHANCED_MODE = True
except ImportError:
    ENHANCED_MODE = False
    import warnings
    warnings.warn("Enhanced cohesion analyzer not available, using legacy detection")
...
class GodObjectDetector:
    """Detects god objects in Python codebases with enhanced cohesion analysis."""
    ...
    def __init__(
        ...
        use_enhanced_mode: bool = ENHANCED_MODE,
        ...
        self.use_enhanced_mode = use_enhanced_mode and ENHANCED_MODE
        self.percentile_threshold = percentile_threshold
        
        if self.use_enhanced_mode:
            self.enhanced_detector = StatisticalGodObjectDetector(
    ...
    def analyze_directory(self, directory: Path) -> List[GodObjectViolation]:
        ...
        if self.use_enhanced_mode:
            return self._analyze_with_enhanced_mode(directory)
        else:
            return self._analyze_with_legacy_mode(directory)
    
    def _analyze_with_enhanced_mode(self, directory: Path) -> List[GodObjectViolation]:
        """Use enhanced cohesion-based analysis."""
        enhanced_findings = self.enhanced_detector.analyze_codebase(directory)
        
        # Convert enhanced findings to legacy format for compatibility
        violations = []
        for finding in enhanced_findings:
            if finding.is_god_object:  # Only include confirmed god objects
                violation = GodObjectViolation(
                    type="class",
                    name=finding.class_name,
                    file_path=finding.file_path,
                    line_count=finding.line_count,
                    method_count=finding.method_count,
                    attribute_count=finding.attribute_count,
                    complexity_score=finding.complexity_z_score,
                    severity=finding.severity.upper(),
                    description=f"God object detected (score: {finding.god_object_score:.2f}, " +
                              f"cohesion: {finding.cohesion_score:.2f}, role: {finding.role.value})",
                    suggestions=finding.refactor_suggestions
                )
                violations.append(violation)
        
        # Also check for god modules using legacy method
        for py_file in directory.rglob("*.py"):
            if self._should_skip_file(py_file):
                continue
            try:
                module_violation = self._check_module_only(py_file)
                if module_violation:
                    violations.append(module_violation)
            except Exception as e:
                print(f"Error analyzing module {py_file}: {e}", file=sys.stderr)
        
        return violations
    
    def _analyze_with_legacy_mode(self, directory: Path) -> List[GodObjectViolation]:
        """Use legacy threshold-based analysis."""
        violations = []
    ...
    def _check_module_only(self, file_path: Path) -> Optional[GodObjectViolation]:
        """Check only for god modules (used in enhanced mode)."""
        try:
            ...
            return self._check_god_module(file_path, lines, tree)
        except Exception:
            return None
