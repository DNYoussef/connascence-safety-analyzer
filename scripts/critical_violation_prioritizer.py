...
class CriticalViolationPrioritizer:
    ...
    def load_critical_violations(self):
        ...
        self.critical_violations = [v for v in data.get("violations", []) if v.get("severity") == "critical"]
    ...
    def prioritize_violations(self):
        ...
        for violation in self.critical_violations:
            ...
            if any(
                keyword in desc + rule_id + violation_type
                for keyword in ["security", "auth", "validation", "injection", "xss", "csrf"]
            ):
                security_critical.append(violation)
            elif any(
                keyword in desc + rule_id + violation_type
                for keyword in ["performance", "memory", "cpu", "optimization", "bottleneck"]
            ):
                performance_critical.append(violation)
            elif any(
                keyword in desc + rule_id + violation_type
                for keyword in ["coupling", "dependency", "circular", "tight"]
            ):
                coupling_critical.append(violation)
            elif any(
                keyword in desc + rule_id + violation_type
                for keyword in ["complexity", "maintainability", "god", "large"]
            ):
    ...
    def _estimate_effort(self, violation):
        ...
        if any(keyword in violation_type + description for keyword in ["security", "coupling", "architecture"]):
            return "8-13 SP (Large)"
        elif any(keyword in violation_type + description for keyword in ["performance", "complexity", "god"]):
    ...
    def generate_executive_summary(self, priority_plan, action_items):
        ...
        total_story_points = sum(effort_map.get(item["estimated_effort"], 5) for item in action_items)
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "priority_breakdown": {
                k: {"count": v["count"], "action_required": v["action_required"], "business_risk": v["business_risk"]}
