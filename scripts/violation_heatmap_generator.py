...
class ViolationHeatmapGenerator:
    ...
    def generate_file_heatmap(self, top_n=50):
        ...
        sorted_files = sorted(file_stats.items(), key=lambda x: x[1]["violations"], reverse=True)[:top_n]
    ...
    def generate_violation_type_heatmap(self):
        ...
        type_stats = defaultdict(
            lambda: {"count": 0, "critical_count": 0, "files_affected": set(), "folders_affected": set()}
        )
        ...
        for violation_type, stats in type_stats.items():
            impact_score = (
                ...
                + (stats["critical_count"] / max(stats["count"], 1)) * 0.4  # Frequency weight
                + (len(stats["files_affected"]) / max_count) * 0.2  # Critical ratio weight  # Spread weight
            )
    ...
    def generate_comprehensive_heatmaps(self):
        ...
        folder_ascii = self.generate_ascii_heatmap(folder_heatmap, "FOLDER VIOLATION HEATMAP (Top 20)")

        file_ascii = self.generate_ascii_heatmap(file_heatmap, "FILE VIOLATION HEATMAP (Top 20)", "heat_intensity")

        type_ascii = self.generate_ascii_heatmap(type_heatmap, "VIOLATION TYPE IMPACT HEATMAP (Top 20)", "impact_score")
