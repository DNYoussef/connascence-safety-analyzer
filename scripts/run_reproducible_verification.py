...
def ensure_analyzer_available():
    ...
    try:
        ...
        print("[SUCCESS] Analyzer imported successfully")
        return True
...
"""
        ...
        return {"test_successful": False, "error": str(e), "analyzer_functional": False}
...
def verify_test_packages(project_root: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                try:
                    ...
                    print(
                        f"[{package.upper()}] {violation_count} violations (target: {target}, within tolerance: {within_tolerance})"
                    )
            ...
            results[package] = {"status": "analysis_failed", "error": str(e)}
    ...
    total_within_tolerance = (
        abs(total_violations - total_target) / total_target <= 0.1 if total_violations > 0 else False
    )
...
def main():
    ...
    results = {"verification_id": verification_id, "timestamp": time.time(), "project_root": str(project_root)}
