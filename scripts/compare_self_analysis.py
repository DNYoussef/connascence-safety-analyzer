...
def compare_results(current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
    ...
    comparison = {"comparison_timestamp": None, "improvements": [], "regressions": [], "metrics_comparison": {}}
    ...
    comparison["metrics_comparison"] = {
        ...
        "nasa_score": {"current": current_nasa, "baseline": baseline_nasa, "change": current_nasa - baseline_nasa},
    }
...
def main():
    ...
    if violations_change > VIOLATION_INCREASE_THRESHOLD:
        significant_regressions.append(
            f"Total violations increased significantly by {violations_change} (threshold: {VIOLATION_INCREASE_THRESHOLD})"
        )

    if violation_percentage_increase > VIOLATION_PERCENTAGE_THRESHOLD:
        significant_regressions.append(
            f"Total violations increased by {violation_percentage_increase:.1%} (threshold: {VIOLATION_PERCENTAGE_THRESHOLD:.1%})"
        )

    if critical_change > CRITICAL_INCREASE_THRESHOLD:
        significant_regressions.append(
            f"Critical violations increased significantly by {critical_change} (threshold: {CRITICAL_INCREASE_THRESHOLD})"
        )

    if nasa_change < -NASA_DECREASE_THRESHOLD:
        significant_regressions.append(
            f"NASA score decreased significantly by {-nasa_change:.3f} (threshold: {NASA_DECREASE_THRESHOLD:.3f})"
        )
