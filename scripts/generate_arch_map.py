...
arch_map = {
    "metadata": {
        "total_files": 762,
        "analyzed_date": "2025-09-23",
        "baseline_nasa_compliance": 19.3
    },
    "module_structure": {},
    "god_objects": [],
    "coupling_analysis": {},
                ...
                if isinstance(node, ast.ClassDef):
                    ...
                    if len(methods) > 20:
                        arch_map["god_objects"].append({
                            ...
                            "priority": "critical" if len(methods) > 50 else "high" if len(methods) > 30 else "medium"
                        })
    except:
        pass
...
arch_map["nasa_root_causes"] = {
    ...
    "rule_4_assertion_density": {
        ...
        "patterns": {
            "0.0% density": "8101 violations - non-test files incorrectly analyzed"
        },
...
arch_map["coupling_analysis"] = {
    "most_coupled": [
        {"file": f, "import_count": c}
        for f, c in sorted(import_counts.items(), key=lambda x: x[1], reverse=True)[:20]
    ],
