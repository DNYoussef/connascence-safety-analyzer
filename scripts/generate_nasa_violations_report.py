...
def main():
    ...
    for filepath, data in file_violations.items():
        ...
        file_rankings.append({
            ...
            'details': {
                'rule_1': data['rule_1'][:3],
                'rule_2': data['rule_2'][:3],
                'rule_4': data['rule_4'][:3]
            }
        })
    ...
    report = {
        ...
        'critical_rules_analysis': {
            'rule_1_simpler_code': {
                ...
                'severity_breakdown': {
                    'high': sum(1 for d in file_rankings for v in d['details']['rule_1'] if v.get('severity') == 'high'),
                    'medium': sum(1 for d in file_rankings for v in d['details']['rule_1'] if v.get('severity') == 'medium')
                },
                'auto_fixable': False,
            ...
            'rule_2_no_gotos': {
                ...
                'severity_breakdown': {
                    'high': sum(d['violations']['rule_2'] for d in file_rankings)
                },
                'auto_fixable': False,
            ...
            'rule_4_assertions': {
                ...
                'severity_breakdown': {
                    'medium': sum(d['violations']['rule_4'] for d in file_rankings)
                },
                'auto_fixable': True,
    ...
    priority_files = sorted(file_rankings[:100], key=lambda x: (x['violations']['total'], x['density']), reverse=True)[:20]
