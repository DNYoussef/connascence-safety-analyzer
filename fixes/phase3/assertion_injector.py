...
class AssertionInjector(ast.NodeTransformer):
    ...
    def _needs_preconditions(self, node: ast.FunctionDef) -> bool:
        ...
        has_assertions = any(
            isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call) and self._is_assertion_call(stmt.value)
    ...
    def _is_assertion_call(self, call: ast.Call) -> bool:
        ...
        if isinstance(call.func, ast.Attribute):
            if isinstance(call.func.value, ast.Name):
                return call.func.value.id == "ProductionAssert" or call.func.value.id == "assert"
        return False
    ...
    def _create_type_check(self, param: str, param_type: str) -> Optional[ast.Expr]:
        ...
        if param_type in type_map:
            ...
            return ast.Expr(
                value=ast.Call(
                    func=ast.Attribute(
                        value=ast.Name(id="ProductionAssert", ctx=ast.Load()), attr="type_check", ctx=ast.Load()
                    ),
                    args=[
    ...
    def _create_not_none_check(self, param: str) -> ast.Expr:
        ...
        return ast.Expr(
            value=ast.Call(
                func=ast.Attribute(
                    value=ast.Name(id="ProductionAssert", ctx=ast.Load()), attr="not_none", ctx=ast.Load()
                ),
                args=[ast.Name(id=param, ctx=ast.Load()), ast.Constant(value=param)],
                keywords=[],
            )
        )
    ...
    def _create_postcondition_check(self, return_value: ast.expr) -> Optional[ast.Expr]:
        ...
        assign = ast.Assign(targets=[ast.Name(id=temp_var, ctx=ast.Store())], value=return_value)
        ...
        assertion = ast.Expr(
            value=ast.Call(
                ...
                args=[ast.Name(id=temp_var, ctx=ast.Load()), ast.Constant(value="return value")],
                keywords=[],
            )
        )
...
class BatchAssertionInjector:
    ...
    def load_violation_data(self) -> List[Dict]:
        ...
        files_with_violations = sorted(data["files_with_violations"], key=lambda x: x["violation_count"], reverse=True)
        ...
        for i, file_info in enumerate(files_to_process, 1):
            ...
            try:
                ...
                if injector.injections_made > 0:
                    ...
                    print("  [SKIP] No assertions needed")
