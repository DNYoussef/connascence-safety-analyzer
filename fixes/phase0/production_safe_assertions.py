...
class ProductionAssert:
    ...
    def type_check(value: Any, expected_type: Type, param_name: str = "parameter"):
        ...
        if not isinstance(value, expected_type):
            raise TypeError(
                f"TYPE: {param_name} must be {expected_type.__name__}, "
                f"got {type(value).__name__}"
            )

    @staticmethod
    def range_check(value: Union[int, float], min_val: Optional[Union[int, float]] = None,
                   max_val: Optional[Union[int, float]] = None, param_name: str = "value"):
        """
                ...
                def wrapped_method(self, *args, **kwargs):
                    ...
                    if not predicate(self):
                        raise AssertionError(
                            f"INVARIANT ({cls.__name__}): {message} after {name}"
                        )
                    return result
...
class LoopInvariant:
    ...
    def check(self):
        ...
        if not self.invariant_func():
            raise AssertionError(
                f"LOOP INVARIANT (iteration {self.iteration_count}): {self.message}"
            )
...
def example_after_migration():
    ...
    def calculate_average(numbers: list) -> float:
        ...
        ProductionAssert.validate(
            numbers,
            lambda lst: all(isinstance(n, (int, float)) for n in lst),
