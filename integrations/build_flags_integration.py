...
class BuildFlagsIntegration:
    ...
    def _detect_gcc(self) -> Optional[CompilerInfo]:
        ...
        try:
            ...
            result = subprocess.run(["gcc", "--version"], check=False, capture_output=True, text=True, timeout=10)
            ...
            # Get path
            path_result = subprocess.run(["which", "gcc"], check=False, capture_output=True, text=True, timeout=5)
            ...
            return CompilerInfo(name="gcc", path=path, version=version, supported_flags=supported_flags)
    ...
    def _detect_clang(self) -> Optional[CompilerInfo]:
        ...
        try:
            result = subprocess.run(["clang", "--version"], check=False, capture_output=True, text=True, timeout=10)
            if result.returncode != 0:
                return None
            ...
            # Get path
            path_result = subprocess.run(["which", "clang"], check=False, capture_output=True, text=True, timeout=5)
            ...
            return CompilerInfo(name="clang", path=path, version=version, supported_flags=supported_flags)
    ...
    def _detect_msvc(self) -> Optional[CompilerInfo]:
        ...
        try:
            ...
            result = subprocess.run(["cl"], check=False, capture_output=True, text=True, timeout=10)
            ...
            if "Microsoft" in result.stderr and "Compiler" in result.stderr:
                ...
                return CompilerInfo(name="msvc", path="cl.exe", version=version, supported_flags=supported_flags)
    ...
    def _detect_build_system(self, project_path: Path) -> Optional[BuildSystemInfo]:
        ...
        if (project_path / "CMakeLists.txt").exists():
            build_system = BuildSystemInfo(
                name="cmake", config_files=[project_path / "CMakeLists.txt"], build_commands=["cmake", "make", "ninja"]
            )
            ...
            build_system = BuildSystemInfo(name="make", config_files=[makefile], build_commands=["make"])
        elif (project_path / "meson.build").exists():
            build_system = BuildSystemInfo(
                name="meson", config_files=[project_path / "meson.build"], build_commands=["meson", "ninja"]
            )
    ...
    def _analyze_compiler_flags(
        self, project_path: Path, compilers: Dict[str, CompilerInfo], build_system: Optional[BuildSystemInfo]
    ) -> Dict[str, Any]:
        """Analyze compiler flags configuration."""
        analysis = {"detected_flags": {}, "missing_flags": {}, "build_system_config": {}, "compilation_test": {}}

        # Analyze build system configuration
        if build_system:
            analysis["build_system_config"] = self._analyze_build_system_config(build_system, project_path)

        # Test actual compilation flags
        for compiler_name, compiler in compilers.items():
            analysis["detected_flags"][compiler_name] = self._get_active_compiler_flags(compiler, project_path)
            ...
            analysis["compilation_test"][compiler_name] = self._test_compilation_with_flags(compiler, project_path)

        return analysis

    def _analyze_build_system_config(self, build_system: BuildSystemInfo, project_path: Path) -> Dict[str, Any]:
    ...
    def _get_active_compiler_flags(self, compiler: CompilerInfo, project_path: Path) -> List[str]:
    ...
    def _check_missing_flags(self, compiler: CompilerInfo, active_flags: List[str]) -> List[str]:
        ...
        if compiler.name in ["gcc", "clang"]:
            ...
            recommended_flags = {"-pedantic", "-Wcast-qual", "-Wcast-align", "-Wwrite-strings", "-Wunused", "-Wshadow"}
        ...
        return {"required_missing": list(missing_required), "recommended_missing": list(missing_recommended)}

    def _test_compilation_with_flags(self, compiler: CompilerInfo, project_path: Path) -> Dict[str, Any]:
...
"""
        ...
        for compiler_name, flags_info in flag_analysis["detected_flags"].items():
            ...
            if compiler_name in ["gcc", "clang"]:
                if "-Wall" not in flags_info:
                    ...
                    compliance["compliant"] = False
            elif compiler_name == "msvc" and "/W4" not in flags_info:
                compliance["violations"].append(f"{compiler_name}: Missing /W4 flag")
                compliance["compliant"] = False

            # Check for static analysis
            static_analysis_flags = {"gcc": ["-fanalyzer"], "clang": ["-analyze", "--analyze"], "msvc": ["/analyze"]}
    ...
    def _generate_flag_recommendations(
        self, flag_analysis: Dict[str, Any], nasa_compliance: Dict[str, Any]
    ) -> List[str]:
        ...
        for compiler_name, missing in flag_analysis["missing_flags"].items():
            if missing.get("required_missing"):
                recommendations.append(f"Add required {compiler_name} flags: {', '.join(missing['required_missing'])}")
