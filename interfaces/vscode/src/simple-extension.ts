...
function debounceAnalyzeFile(document: vscode.TextDocument) {
    const config = vscode.workspace.getConfiguration('connascence');
...
function performBasicAnalysis(document: vscode.TextDocument): vscode.Diagnostic[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (magicNumberMatch && !line.includes('//') && !line.includes('#')) {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                `Magic number detected: ${magicNumberMatch[0]}. Consider extracting to a named constant.`,
        ...
        if ((line.includes('def ') || line.includes('function ')) && (line.match(/,/g) || []).length >= 4) {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                'Function has too many parameters (Connascence of Position). Consider using a parameter object.',
                vscode.DiagnosticSeverity.Warning
            );
        ...
        if (line.includes('isinstance') || line.includes('typeof')) {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                'Potential Connascence of Type detected. Consider polymorphism or duck typing.',
        ...
        if ((line.includes('for ') || line.includes('while ')) && indentation >= 8) {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                'Deeply nested loop detected (potential Connascence of Algorithm). Consider refactoring.',
                vscode.DiagnosticSeverity.Warning
            );
