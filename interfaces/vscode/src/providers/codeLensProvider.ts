 ...
 */
export class ConnascenceCodeLensProvider implements vscode.CodeLensProvider {
    ...
    private async getFileMetrics(document: vscode.TextDocument): Promise<vscode.Command | null> {
        try {
    ...
    private async getFunctionMetrics(document: vscode.TextDocument): Promise<vscode.CodeLens[]> {
        const codeLenses: vscode.CodeLens[] = [];
    ...
    private async getClassMetrics(document: vscode.TextDocument): Promise<vscode.CodeLens[]> {
        const codeLenses: vscode.CodeLens[] = [];
        const text = document.getText();
    ...
    private async getViolationWarnings(document: vscode.TextDocument): Promise<vscode.CodeLens[]> {
        const codeLenses: vscode.CodeLens[] = [];
    ...
    private async analyzeDocument(document: vscode.TextDocument): Promise<any> {
    ...
    private async analyzeFunctionAtPosition(document: vscode.TextDocument, position: vscode.Position): Promise<any> {
    ...
    private async analyzeClassAtPosition(document: vscode.TextDocument, position: vscode.Position): Promise<any> {
        ...
        for (let i = startLine + 1; i < lines.length && i < startLine + 100; i++) {
            const line = lines[i];
            if (line.includes('def ')) methodCount++;
            if (line.includes('import ') || line.includes('from ')) dependencyCount++;
            if (line.match(/^class\s+/) && i > startLine) break; // Next class
        }
    ...
    private calculateCyclomaticComplexity(lines: string[]): number {
        ...
        for (const line of lines) {
            ...
            if (line.includes('elif ')) complexity++;
            if (line.includes('for ')) complexity++;
            if (line.includes('while ')) complexity++;
            if (line.includes('except ')) complexity++;
    ...
    private calculateQualityScore(analysis: any): number {
        ...
        const violationWeight = violations.reduce((sum: number, v: any) => {
            switch (v.severity) {
    ...
    private formatFunctionMetrics(analysis: any): string {
    ...
    private formatClassMetrics(analysis: any): string {
