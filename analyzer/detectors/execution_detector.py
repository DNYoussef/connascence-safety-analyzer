...
class ExecutionDetector(DetectorBase):
    ...
    def _track_name_usage(self, node: ast.Name) -> None:
        ...
        if isinstance(node.ctx, ast.Store):
            ...
            var_name = node.id
            if any(keyword in var_name.lower() for keyword in ["init", "setup", "config", "state", "cache", "buffer"]):
    ...
    def _track_function_call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name):
            ...
            if any(
                ...
                for keyword in ["init", "setup", "start", "stop", "open", "close", "connect", "disconnect"]
            ):
    ...
    def _check_exception_flow_coupling(self) -> None:
        ...
        if len(self.exception_handlers) > 5:
            ...
            self._create_exception_flow_violation(representative_node, len(self.exception_handlers))
    ...
    def _check_side_effect_coupling(self) -> None:
        ...
        if len(side_effect_calls) > 8:  # Many side-effect calls
            ...
            self._create_side_effect_violation(representative_node, len(side_effect_calls))
    ...
    def _check_import_order_dependencies(self) -> None:
        ...
        if len(self.import_statements) > 0:
            ...
            early_code = [
                node
                for node in self.function_calls + self.global_assignments
                if hasattr(node, "lineno") and node.lineno < last_import_line
            ]

            if len(early_code) > 0:
                representative_node = self.import_statements[-1]
                self._create_import_order_violation(representative_node, len(early_code), last_import_line)
    ...
    def _create_global_state_violation(self, node: ast.AST, global_assignments: int, stateful_vars: int) -> None:
    ...
    def _create_initialization_order_violation(self, node: ast.AST, init_functions: List[str]) -> None:
    ...
    def _create_exception_flow_violation(self, node: ast.AST, handler_count: int) -> None:
        ...
        self.violations.append(
            ConnascenceViolation(
                ...
                code_snippet=self.get_code_snippet(node),
                context={"violation_type": "exception_flow_coupling", "handler_count": handler_count},
            )
        )

    def _create_side_effect_violation(self, node: ast.AST, side_effect_count: int) -> None:
        ...
        self.violations.append(
            ConnascenceViolation(
                ...
                code_snippet=self.get_code_snippet(node),
                context={"violation_type": "side_effect_coupling", "side_effect_count": side_effect_count},
            )
        )

    def _create_import_order_violation(self, node: ast.AST, early_code_count: int, last_import_line: int) -> None:
