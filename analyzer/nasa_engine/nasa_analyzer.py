...
try:
    from ..optimization.file_cache import cached_ast_tree, cached_file_content, get_global_cache
    ...
    def _get_default_nasa_config(self) -> Dict:
        ...
        config = {
            "rules": {
                "nasa_rule_1": {
                    "severity": "critical",
                    "violations": [{"type": "goto_statement"}, {"type": "recursive_function"}],
                },
                ...
                "nasa_rule_5": {
                    "severity": "high",
                    "violations": [{"type": "insufficient_assertions", "threshold": 2}],
                },
    ...
    def _check_rule_3_heap_usage(self, file_path: str) -> None:
        ...
        for malloc_call in self.malloc_calls:
            violation = ConnascenceViolation(
                ...
                code_snippet="Memory allocation call",
        ...
        for func in self.function_definitions:
            ...
            if func_assertions < 2 and self._calculate_function_lines(func) > 5:  # Only check non-trivial functions
                violation = ConnascenceViolation(
                    ...
                    context={
                        "nasa_rule": "rule_5",
                        "violation_type": "insufficient_assertions",
    ...
    def _check_rule_6_variable_scope(self, file_path: str) -> None:
        ...
        if len(self.global_variables) > 20:
            violation = ConnascenceViolation(
                ...
                context={
                    "nasa_rule": "rule_6",
                    "violation_type": "excessive_global_variables",
                ...
                if isinstance(node, ast.Expr) and isinstance(node.value, ast.Call):
                    ...
                    if isinstance(node.value.func, ast.Name):
                        ...
                        if func_name not in [
                            ...
                            "assert",
                            "len",
                        ]:  # Common functions where return value is often ignored
