...
class SOC2Collector:
    ...
    def collect_access_control_evidence(self) -> List[SOC2Evidence]:
        ...
        if gitignore_path.exists():
            ...
            secrets_protected = any(pattern in gitignore_content for pattern in [
                '*.key', '*.pem', '.env', 'secrets', 'credentials'
            ])

            evidence.append(SOC2Evidence(
                control_id="CC6.1",
    ...
    def collect_change_management_evidence(self) -> List[SOC2Evidence]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                reviewed_changes = len([c for c in commits if any(keyword in c.lower() for keyword in ['review', 'approved', 'merge'])])
    ...
    def collect_monitoring_evidence(self) -> List[SOC2Evidence]:
        ...
        evidence.append(SOC2Evidence(
            ...
            data={
                ...
                "automated_testing_enabled": test_files_found > 0
            }
        ))
    ...
    def generate_report(self, period_start: Optional[str] = None,
                       period_end: Optional[str] = None) -> SOC2Report:
        if not period_end:
