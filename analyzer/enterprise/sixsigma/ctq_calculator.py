...
class CTQCalculator:
        ...
        try:
            ...
            self.ctq_definitions = {
                ...
                'tests_mutation': CTQDefinition(
                    ...
                    spec=self._find_ctq_spec(ctqs, 'tests_mutation', {'require_pass': True, 'min_mutation_changed': 0.60})
                ),
            ...
            self.ctq_definitions = self._get_default_definitions(); return self.ctq_definitions
    ...
    def _calculate_ctq_score(self, ctq_name: str, data: Dict[str, Any],
                            definition: CTQDefinition) -> Tuple[float, float]:
        actual = 0.0

        if ctq_name == 'security':
            security_data = data.get('security', {})
            actual = security_data.get('score',
                max(0, 95 - security_data.get('critical', 0) * 20 - security_data.get('high', 0) * 5))
    ...
    async def monitor(self, metrics: Dict[str, Any]) -> MonitoringResults:
        ...
        critical_ctqs = [
            {'name': name, **asdict(score)}
            for name, score in results.ctq_scores.items()
