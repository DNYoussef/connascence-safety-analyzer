...
class SixSigmaAnalyzer:
    ...
    def analyze_violations(
        self, violations: List[Dict[str, Any]], file_path: Optional[Path] = None
    ) -> SixSigmaAnalysisResult:
        """
        ...
        suggestions = self._generate_improvement_suggestions(
            metrics.dpmo, metrics.sigma_level, violations_by_type, violations_by_severity
        )
    ...
    def _calculate_ctq_metrics(self, violations_by_type: Dict[str, int], total_violations: int) -> Dict[str, float]:
        """Calculate Critical To Quality metrics"""
        if total_violations == 0:
            return dict.fromkeys(self.CTQ_WEIGHTS.keys(), 0.0)
    ...
    def _generate_improvement_suggestions(
        ...
        if sigma_level < self.target_level["sigma"]:
            ...
            suggestions.append(f"Improve sigma level by {gap:.1f} to reach {self.target_level['sigma']:.1f} target")
    ...
    def analyze_codebase(self, violations_by_file: Dict[Path, List[Dict]]) -> Dict[str, Any]:
        ...
        dmaic_plan = self._generate_dmaic_plan(total_dpmo, avg_sigma, total_by_type, total_by_severity)
        ...
        return {
            ...
            "file_results": [
                {"file": str(r.file_path), "dpmo": r.dpmo, "sigma_level": r.sigma_level, "quality": r.quality_level}
    ...
    def _generate_dmaic_plan(
        ...
        return {
            ...
            "control": {
                ...
                "control_limits": {"upper": self.target_level["dpmo"], "lower": 0},
    ...
    def _create_impact_matrix(self, violations_by_type: Dict[str, int]) -> List[Dict[str, Any]]:
        ...
        for vtype, count in violations_by_type.items():
            ...
            matrix.append(
                {"type": vtype, "count": count, "impact": round(impact, 2), "effort": effort, "quadrant": quadrant}
            )
            ...
            "analysis_results": [
                {
                    ...
                    "violations": {"by_type": r.violations_by_type, "by_severity": r.violations_by_severity},
