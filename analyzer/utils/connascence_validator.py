...
class ConnascenceMetrics:
    ...
    def total_violations(self) -> int:
        ...
        return (
            self.hardcoded_values_count
            + self.magic_literals_count
            + self.direct_constructor_calls
            + self.duplicate_algorithms
            + self.position_coupling_instances
            + self.configuration_scattering
            + self.error_handling_patterns
            + self.interface_inconsistencies
        )
...
class ConnascenceValidator:
    ...
    def _generate_recommendations(self, metrics: ConnascenceMetrics) -> List[str]:
        ...
        if metrics.magic_literals_count > 5:
            recommendations.append(f"Extract {metrics.magic_literals_count} magic literals to named constants")
        ...
        if metrics.duplicate_algorithms > 0:
            recommendations.append(f"Extract {metrics.duplicate_algorithms} duplicate algorithm patterns")
