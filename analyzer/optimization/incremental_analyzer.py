...
class IncrementalAnalyzer:
    ...
    def analyze_changes(self, 
                       ...
                       force_baseline: bool = False) -> IncrementalAnalysisResult:
        """
        ...
        logger.info(f"Incremental analysis complete: {analysis_time:.2f}s saved {time_saved:.2f}s "
    ...
    def create_baseline(self) -> Dict[str, Any]:
        ...
        baseline = {
            ...
            'quality_metrics': {
                ...
                'critical_violations': len([v for v in result.get('violations', []) if v.get('severity') == 'critical'])
            }
        }
    ...
    def get_analysis_report(self, result: IncrementalAnalysisResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_metrics': {
                ...
                'performance_improvement_percent': (result.time_saved_seconds / (result.analysis_time_seconds + result.time_saved_seconds)) * 100,
                'cache_hit_rate_percent': result.cache_hit_rate,
                'throughput_files_per_second': result.analyzed_files_count / result.analysis_time_seconds if result.analysis_time_seconds > 0 else 0
            },
    ...
    def optimize_for_ci_cd(self) -> Dict[str, Any]:
        ...
        if hasattr(self, 'dependency_graph') and self.dependency_graph:
            high_fan_out_files = [
                file_path for file_path, deps in self.dependency_graph.items()
                if len(deps) > 20
            ]
    ...
    def _compare_with_baseline(self, current_results: Dict[str, Any], baseline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        new_violations = [
            v for sig, v in current_sigs.items() 
            if sig not in baseline_sigs
        ]
        
        resolved_violations = [
            v for sig, v in baseline_sigs.items()
    ...
    def _has_valid_baseline(self) -> bool:
        ...
        try:
            baseline_time = time.mktime(time.strptime(
                self.baseline_results.get('timestamp', ''),
                '%Y-%m-%d %H:%M:%S'
            ))
    ...
    def _update_baseline(self, current_results: Dict[str, Any], commit_hash: Optional[str]):
        ...
        self.baseline_results = {
            ...
            'quality_metrics': {
                ...
                'critical_violations': len([v for v in current_results.get('violations', []) if v.get('severity') == 'critical'])
            }
        }
