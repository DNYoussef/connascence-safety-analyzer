...
from .memory_monitor import MemoryLeakDetector
...
class StreamingPerformanceMonitor:
    ...
    def __init__(self, max_metrics_history: int = 1000, memory_sample_interval: float = 1.0):
        """
    ...
    def start_session(self, session_id: str, watched_files: int = 0) -> None:
        ...
        with self._lock:
            session = StreamingSessionStats(
                session_id=session_id, start_time=time.time(), total_files_watched=watched_files
            )
    ...
    def end_session(self, session_id: str) -> StreamingSessionStats:
        ...
        with self._lock:
            ...
            if self.current_metrics.analysis_latency_ms:
                session.average_latency_ms = sum(self.current_metrics.analysis_latency_ms) / len(
                    self.current_metrics.analysis_latency_ms
                )
    ...
    def record_memory_sample(self, memory_mb: float) -> None:
        ...
        with self._lock:
            ...
            # Update peak memory
            self.current_metrics.memory_peak_mb = max(self.current_metrics.memory_peak_mb, memory_mb)
    ...
    def _update_throughput(self) -> None:
        ...
        current_time = time.time()
        recent_events = [
            (t, _) for event_list in self.event_timings.values() for t, _ in event_list if current_time - t <= 60.0
        ]
