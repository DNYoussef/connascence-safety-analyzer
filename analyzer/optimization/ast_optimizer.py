...
from typing import Dict, List, Set, Any, Optional, Union, Callable, Iterator
...
class OptimizedASTVisitor(ast.NodeVisitor):
    ...
    def _subtree_might_contain_targets(self, node: ast.AST, targets: Union[Set[str], Set[type]]) -> bool:
        ...
        if targets and isinstance(next(iter(targets)), type):
            target_names = {t.__name__ for t in targets}
        else:
...
class ConnascencePatternOptimizer:
    ...
    def _generate_description(self, node: ast.AST, connascence_type: str) -> str:
        ...
        descriptions = {
            ...
            'connascence_of_position': f"Position coupling detected in function call",
