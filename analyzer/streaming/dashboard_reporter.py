...
from .result_aggregator import AggregatedResult, get_global_stream_aggregator
...
class DashboardReporter:
    ...
    def __init__(
        ...
        self.violation_trends: Dict[str, deque] = defaultdict(lambda: deque(maxlen=max_metrics))
    ...
    def add_metrics_sample(self, violations: int, files_analyzed: int, performance_data: Dict[str, Any]) -> None:
    ...
    def add_violation_trend(self, violation_type: str, count: int, cumulative_count: int, files_affected: int) -> None:
    ...
    def get_metrics_history(self, time_window_minutes: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def get_violation_trends(
        self, violation_type: Optional[str] = None, time_window_minutes: int = 30
        ...
        with self._lock:
            ...
            for v_type in violation_types:
                if v_type in self.violation_trends:
                    filtered_trends = [
                        asdict(trend) for trend in self.violation_trends[v_type] if trend.timestamp >= cutoff_time
                    ]
    ...
    def _generate_summary(
        self, aggregated_result: AggregatedResult, performance_report: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_violations_data(self, aggregated_result: AggregatedResult) -> Dict[str, Any]:
        ...
        top_violations = sorted(violation_breakdown.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    def _calculate_system_status(self, performance_report: Dict[str, Any]) -> str:
        ...
        # Check for warnings
        if (
            current_metrics.get("queue_depth", 0) > 100
            or current_metrics.get("cache_hit_rate", 100) < 30
            or current_metrics.get("average_latency_ms", 0) > 10000
        ):
            return "warning"
    ...
    def _calculate_analysis_type_breakdown(self, file_analysis_history: Dict[str, List[Dict]]) -> Dict[str, int]:
...
def add_dashboard_metrics_sample(violations: int, files_analyzed: int, performance_data: Dict[str, Any]) -> None:
