...
class FileDelta:
    ...
    def __post_init__(self):
        """Validate delta data."""
        assert self.change_type in ["created", "modified", "deleted"], f"Invalid change_type: {self.change_type}"
...
class IncrementalCache:
    ...
    def __init__(
        self, max_partial_results: int = 5000, max_dependency_nodes: int = 10000, cache_retention_hours: float = 24.0
    ):
        """
    ...
    def track_file_change(
        self, file_path: Union[str, Path], old_content: Optional[str] = None, new_content: Optional[str] = None
    ) -> FileDelta:
        """
    ...
    def get_partial_result(
        self, file_path: Union[str, Path], result_type: str, current_hash: Optional[str] = None
    ...
    def _update_dependency_tracking(self, file_path: str, dependencies: Set[str]) -> None:
        ...
        if file_path not in self._dependency_graph:
            self._dependency_graph[file_path] = DependencyNode(
                file_path=file_path, content_hash=self._file_hashes.get(file_path, ""), last_analyzed=time.time()
            )
        ...
        for dep_path in dependencies - old_dependencies:
            if dep_path not in self._dependency_graph:
                self._dependency_graph[dep_path] = DependencyNode(
                    file_path=dep_path, content_hash=self._file_hashes.get(dep_path, ""), last_analyzed=0
                )
    ...
    def _evict_old_partial_results(self) -> None:
        ...
        sorted_results = sorted(self._partial_results.items(), key=lambda x: x[1].created_at)
    ...
    def get_files_needing_analysis(self, file_paths: List[str], analysis_type: str = "violations") -> List[str]:
        """
