...
from analyzer.check_connascence import ConnascenceAnalyzer as LegacyAnalyzer, ConnascenceDetector
...
'''
        ...
        flagged_values = set(str(v.context.get('literal_value', '')) for v in magic_violations)
...
'''
    ...
    def test_file_analysis_behavior(self):
        ...
        should_skip_patterns = [
            "__pycache__/cache.py",
            ".git/config",
            "node_modules/lib.js", 
            "build/output.o",
            "very_large_file.py"  # Would be skipped by size check
        ]
    ...
    def test_multi_language_detection(self):
        ...
        js_code = '''
function badFunction(a, b, c, d, e, f, g) {  // Too many params
    var timeout = 42;  // Magic number
    var buffer = 1024; // Magic number  
    return a + b + c + d + e + f + g + timeout + buffer;
}
'''
        
        # C-style code
        c_code = '''
int bad_function(int a, int b, int c, int d, int e, int f, int g) {
    int timeout = 42;  /* Magic number */
    int buffer = 1024; /* Magic number */
    return a + b + c + d + e + f + g + timeout + buffer;
}
'''
        ...
        try:
            from analyzer.language_strategies import JavaScriptStrategy, CStrategy
