...
"""
        ...
        self.assertEqual(
            len(line_type_combinations), len(violations), "Each violation should be unique (line, type) combination"
        )
    ...
    def test_detector_error_handling(self):
        ...
        with patch.object(
            self.detector.timing_detector, "detect_violations", side_effect=Exception("Test detector error")
        ):
    ...
    def test_detector_performance_isolation(self):
        ...
        with patch.object(self.detector.timing_detector, "detect_violations", side_effect=slow_detector):
            test_code = "def test(): pass"
            tree = ast.parse(test_code)
...
"""
        ...
        for violation in violations:
            ...
            if "timing" in violation.type:
                ...
                self.assertTrue(
                    any(
                        key in str(violation.context) or key in violation.description
