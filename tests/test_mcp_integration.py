...
"""
            ...
            result = self.server.execute_tool("scan_path", {"path": temp_dir, "policy": "default"})
    ...
    def test_explain_finding_tool(self):
        ...
        result = self.server.execute_tool("explain_finding", {"violation_id": "test_violation_123"})
    ...
    def test_propose_autofix_tool(self):
        ...
        test_violations = [{"id": "violation_1", "type": "CoM", "severity": "medium"}]

        result = self.server.execute_tool(
            "propose_autofix", {"violations": test_violations, "safety_level": "conservative"}
        )
...
class TestMCPAsyncInterface:
    ...
    async def test_async_scan_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = await self.server.scan_path({"path": temp_dir, "policy_preset": "default"})
    ...
    async def test_async_explain_finding(self):
        ...
        result = await self.server.explain_finding({"violation_id": "test_123"})
    ...
    async def test_async_propose_autofix(self):
        ...
        result = await self.server.propose_autofix(
            {"violations": [{"id": "test", "type": "CoM"}], "safety_level": "safe"}
        )
...
class TestMCPRateLimiting:
    ...
    def test_rate_limit_enforcement(self):
        ...
        for i in range(5):
            result = self.server.execute_tool("explain_finding", {"violation_id": f"test_{i}"})
            assert result["success"] is True

        # Should fail when limit exceeded
        with pytest.raises(Exception, match="Rate limit exceeded"):
            self.server.execute_tool("explain_finding", {"violation_id": "test_overflow"})

    def test_rate_limit_reset(self):
        """Test rate limit resets over time."""
        # Use up rate limit
        for i in range(5):
            self.server.execute_tool("explain_finding", {"violation_id": f"test_{i}"})

        # Clear rate limiter manually (simulating time passage)
        self.server.rate_limiter.requests.clear()

        # Should work again
        result = self.server.execute_tool("explain_finding", {"violation_id": "test_after_reset"})
        assert result["success"] is True
...
class TestMCPFallbackMode:
    ...
    def test_standalone_analyzer_fallback(self):
        ...
        with patch("subprocess.run", side_effect=FileNotFoundError("MCP server not available")):
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                result = server.execute_tool("scan_path", {"path": temp_dir})
    ...
    def test_graceful_degradation(self):
        ...
        config = {"enable_graceful_degradation": True, "offline_mode": True}
...
class TestMCPConfigurationValidation:
    ...
    def test_valid_configuration(self):
        ...
        config = {"max_requests_per_minute": 100, "enable_audit_logging": True, "allowed_paths": ["/safe/path"]}
...
class TestMCPMetrics:
    ...
    def test_metrics_collection(self):
        ...
        self.server.execute_tool("explain_finding", {"violation_id": "test_1"})
...
class TestConnascenceViolationModel:
    ...
    def test_violation_to_dict_conversion(self):
        ...
        violation = ConnascenceViolation(id="test_456", rule_id="CON_CoP", connascence_type="CoP", severity="high")
