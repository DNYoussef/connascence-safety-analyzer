...
'''
    ...
    def test_unified_visitor_data_collection(self):
        ...
        print(f"✓ Collected data: {len(collected_data.functions)} functions, "
        ...
        for node in ast.walk(functions_in_visitor):
            if isinstance(node, ast.FunctionDef):
                if hasattr(node, 'end_lineno') and hasattr(node, 'lineno'):
                    ...
                    self.assertLessEqual(func_lines, 60, 
                                       f"Function {node.name} has {func_lines} lines (NASA Rule 4 violation)")
    ...
    def test_performance_improvement_estimation(self):
        ...
        self.assertGreaterEqual(improvement_percentage, 85.0, 
