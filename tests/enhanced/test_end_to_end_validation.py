        ...
        if channel == "email":
            ...
            return self._send_sms(user_id, message)
                ...
                expected_correlations=[
                    MockCorrelation("user_service", "order_service", "cross_service_dependency", 0.85, "Services tightly coupled through user status validation", "high"),
                    MockCorrelation("order_service", "notification_service", "execution_flow_dependency", 0.75, "Order completion triggers notification flow", "medium")
                ],
                expected_recommendations=[
                    MockSmartRecommendation("introduce_user_status_enum", "Create UserStatus enum to reduce meaning connascence", "medium", ["user_service.py", "order_service.py"]),
                    MockSmartRecommendation("implement_service_interface", "Extract UserService interface to reduce type connascence", "high", ["order_service.py"])
                ],
...
class ModernAdapter:
    ...
    def process(self, data, output_format="JSON"):
        ...
        return self._modernize_output(result, output_format)
                ...
                expected_correlations=[
                    MockCorrelation("legacy_processor", "modern_adapter", "adaptation_coupling", 0.95, "Adapter tightly coupled to legacy implementation details", "critical")
                ],
                expected_recommendations=[
                    MockSmartRecommendation("extract_processor_interface", "Create ProcessorInterface to decouple adapter", "critical", ["legacy_processor.py", "modern_adapter.py"]),
                    MockSmartRecommendation("introduce_format_enum", "Replace string formats with enum", "high", ["legacy_processor.py"])
                ],
                performance_requirements={
...
class Module{i}:
    ...
    def _process_type_0(self, data, format):
        ...
        self._setup_test_files(temp_project_directory, [
            {filename: content} for filename, content in large_files.items()
        ])
    ...
    def _validate_findings(self, result: Dict[str, Any], expected_findings: List[Dict[str, Any]]):
        ...
        for expected in expected_findings:
            matching_findings = [
                f for f in findings
                if f.get("type") == expected["type"] and expected["file"] in str(f.get("file", ""))
            ]
    ...
    def _validate_recommendations(self, result: Dict[str, Any], expected_recommendations: List[MockSmartRecommendation]):
