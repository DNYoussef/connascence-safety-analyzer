...
'''
        ...
        if "errors" in result:
            network_errors = [e for e in result["errors"] if "network" in e.get("message", "").lower() or "timeout" in e.get("message", "").lower()]
...
'''
        ...
        if "warnings" in result:
            recovery_warnings = [w for w in result["warnings"] if "recover" in w.get("message", "").lower() or "regenerat" in w.get("message", "").lower()]
        
        # Should attempt to regenerate or recover
        assert "analysis_attempted" in result or len(result.get("findings", [])) >= 0, "Should attempt analysis recovery"
        ...
        for interface in interfaces:
            ...
            with patch('json.dumps') as mock_json_dumps:
                ...
                try:
                    ...
                    assert "error" in formatted_result or "fallback" in formatted_result, \
                        f"{interface}: Should handle communication failure"
                    
                except Exception as e:
                    # Should not propagate communication errors
                    assert "Communication failure" not in str(e), f"{interface}: Should not propagate communication errors"
...
'''
    ...
    def _format_for_interface_with_error_handling(self, result: Dict[str, Any], interface: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "error": f"Interface communication failure: {str(e)}",
