...
class TestReportGenerationWorkflows:
    ...
    def test_json_report_comprehensive_validation(self, sample_project_with_violations, report_workflow_validator):
        ...
        content_analysis = {
            ...
            'violation_types_found': len(set(v.get('connascence_type', '') for v in violations)),
            'severities_found': len(set(v.get('severity', {}).get('value', '') for v in violations)),
            ...
            if format_type in ["json", "sarif"]:
                ...
                if format_type == "json":
                    ...
                    format_data[format_type] = {
                        ...
                        'violation_types': set(v.get('connascence_type', '') for v in violations)
                    }
                else:  # sarif
                    results = data.get('runs', [{}])[0].get('results', [])
                    format_data[format_type] = {
                        'violation_count': len(results),
                        'rule_ids': set(r.get('ruleId', '') for r in results)
                    }
            else:
