...
exports.ConnascenceCompletionProvider = void 0;
const vscode = __importStar(require("vscode"));
/**
 * IntelliSense completion provider for connascence-aware code suggestions
 */
class ConnascenceCompletionProvider {
    ...
    getConnascenceCompletions(textBeforeCursor, position) {
        const completions = [];
        ...
        if (textBeforeCursor.includes('def ') && textBeforeCursor.includes('(') && !textBeforeCursor.includes('):')) {
            completions.push({
                ...
                documentation: new vscode.MarkdownString('Add type annotations to reduce **Connascence of Type** violations\\n\\n' +
        ...
        if (/\b\d+\b|\b['"`][^'"`]*['"`]\b/.test(textBeforeCursor)) {
            completions.push({
                ...
                documentation: new vscode.MarkdownString('Extract magic literals to constants to reduce **Connascence of Meaning**\\n\\n' +
        ...
        if (textBeforeCursor.includes('def ') && (textBeforeCursor.match(/,/g) || []).length >= 3) {
            completions.push({
                ...
                documentation: new vscode.MarkdownString('Replace long parameter lists with parameter objects to reduce **Connascence of Position**\\n\\n' +
    ...
    getPythonSafePatterns(textBeforeCursor, position) {
        const completions = [];
        ...
        if (textBeforeCursor.includes('class ')) {
            completions.push({
                ...
                documentation: new vscode.MarkdownString('Use dataclass to reduce constructor coupling and improve type safety'),
        ...
        if (textBeforeCursor.includes('from enum import') || textBeforeCursor.includes('import enum')) {
            completions.push({
                ...
                documentation: new vscode.MarkdownString('Use Enum to replace magic strings and reduce Connascence of Meaning'),
        ...
        if (textBeforeCursor.includes('with ')) {
            completions.push({
                ...
                insertText: new vscode.SnippetString('def ${1:context_manager}():\\n' +
                    ...
                    '    finally:\\n' +
                    '        ${4:# cleanup}'),
                documentation: new vscode.MarkdownString('Create context manager to reduce temporal coupling and ensure cleanup'),
    ...
    getJavaScriptSafePatterns(textBeforeCursor, position) {
        const completions = [];
        ...
        if (textBeforeCursor.includes('function ') || textBeforeCursor.includes('const ') && textBeforeCursor.includes('=')) {
            completions.push({
                ...
                documentation: new vscode.MarkdownString('Use object destructuring to reduce parameter position coupling'),
        ...
        if (textBeforeCursor.includes('const ') && !textBeforeCursor.includes('as const')) {
            completions.push({
                ...
                insertText: ' as const',
                documentation: new vscode.MarkdownString('Add const assertion to improve type inference and reduce type coupling'),
        ...
        if (textBeforeCursor.includes('.') && !textBeforeCursor.includes('?.')) {
            completions.push({
                ...
                insertText: '?.',
                documentation: new vscode.MarkdownString('Use optional chaining to reduce structural coupling and prevent runtime errors'),
    ...
    getRefactoringCompletions(textBeforeCursor, position) {
        const completions = [];
        ...
        if (textBeforeCursor.includes('new ') || textBeforeCursor.includes('class ')) {
            completions.push({
                ...
                kind: vscode.CompletionItemKind.Method,
                insertText: new vscode.SnippetString('static create${1:Type}(${2:config}) {\\n' +
                    '    return new ${3:ClassName}(${4:...config});\\n}'),
                documentation: new vscode.MarkdownString('Use factory method to reduce constructor coupling and centralize creation logic'),
        ...
        if ((textBeforeCursor.match(/,/g) || []).length >= 4) {
            completions.push({
                ...
                kind: vscode.CompletionItemKind.Class,
                insertText: new vscode.SnippetString('class ${1:Builder} {\\n' +
                    ...
                    '    }\\n}'),
                documentation: new vscode.MarkdownString('Use builder pattern to reduce parameter coupling and improve readability'),
...
exports.ConnascenceCompletionProvider = ConnascenceCompletionProvider;
