...
class FileWatcherService {
    ...
    async analyzeOpenDocuments() {
        const openDocs = vscode.workspace.textDocuments.filter(doc => this.shouldAnalyzeFile(doc.uri));
    ...
    async analyzeWorkspace() {
        ...
        for (const pattern of patterns) {
            try {
                const files = await vscode.workspace.findFiles(pattern, `{${excludePatterns.join(',')}}`, 1000 // Limit to 1000 files per pattern
                );
